/*
Tree-based Group Diffie-Hellman (TGDH) - Core Library

Defines the Node struct, file I/O helpers, hex utilities, and all primitives
used by the setup/join/leave/merge/refresh executables: DH parameter loading,
hex/byte/BIGNUM conversions, SHA-256, leaf-blinded key computation, tree
construction, bottom-up internal key derivation, output writers, and tree
teardown.

Ownership model: once build_tree() returns, the key tree owns every leaf and
internal byte buffer. Callers must NULL out their secret_bytes[] and
blind_bytes[] entries before invoking free_tree() to avoid double-frees.

Crypto primitives: OpenSSL BIGNUM modular exponentiation, SHA-256.
DH parameters: RFC 2409 Group 2 (1024-bit MODP), g = 2.
Reference: https://www.openssl.org/docs/manmaster/man3/
*/
...
// Read an entire text file into a heap buffer; trim trailing whitespace/newlines
...
// Write a null-terminated string to filename
...
// Read an entire text file and parse its contents as a decimal integer
...
// Read every non-blank line of filename into a freshly allocated array of strings.
// Returns the line count. Caller owns each line and the outer array.
int Read_Lines(const char *filename, char ***lines_out) {
    ...
    while (fgets(buf, sizeof(buf), f)) {
        ...
        while (len > 0 && (buf[len-1] == '\n' || buf[len-1] == '\r' || buf[len-1] == ' '))
            buf[--len] = '\0';
        if (len == 0) continue;  // drop empty lines
...
// Print a labeled buffer as a continuous lowercase hex string
...
// Encode a byte buffer as a null-terminated lowercase hex string (output must be 2*len + 1 bytes)
...
/*
TGDH primitives
The tree owns all key byte buffers. After build_tree() returns, callers must
NULL out their secret_bytes[] / blind_bytes[] entries so free_tree() can
release everything without double-freeing.
*/
