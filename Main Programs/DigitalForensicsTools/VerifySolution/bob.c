...
Bob (Auditor) Program Description

Bob acts as the Auditor in the forward-secure and aggregate digital
forensics system. His responsibility is to verify integrity and
authenticity of the ciphertext log before performing any decryption.

The system provides:
    - Forward-secure key evolution
    - Individual message authentication
    - Aggregate authentication (O(1) final tag size)
    - Compromise-resilient symmetric encryption

Bob verifies that the ciphertext log has not been tampered with.
Decryption only occurs if authentication succeeds.

Technical Details:
    - PRNG using ChaCha20 (to derive initial key)
    - SHA-256 (for hash chaining and key evolution)
    - HMAC-SHA256 (for per-message authentication)
    - AES-CTR (for decryption)
    - OpenSSL crypto library

Bob communicates with Alice through text files.
*/
...
int main(int argc, char *argv[]) {
    ...
    while(fgets((char*)buffer, MAX_MESSAGE_LENGTH, fp) != NULL){
        ...
        Hex_to_Bytes((char*)buffer, messages[i].cipherText, message_len);
        messages[i++].cipherTextLen = message_len / 2;
    
        // Skip empty buffer
        fgets((char*)buffer, MAX_MESSAGE_LENGTH, fp);
    }
    ...
    for(i = 0; i < message_amount; i++){
        ...
        // Hash aggregate HMAC -> next key
        Compute_SHA256(messages[i].aggregateHmac, KEY_LENGTH, messages[i + 1].key);
    }
    ...
    if((memcmp(correct_aggregated_HMAC, messages[message_amount - 1].aggregateHmac, HMAC_LENGTH) == 0)){
        ...
        Write_File("Plaintexts.txt", (char*)buffer, i * (messages[i - 1].plainTextLen + 1) - 1);
    }
    else{
