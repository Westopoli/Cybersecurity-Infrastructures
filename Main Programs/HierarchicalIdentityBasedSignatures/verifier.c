/*
 * verifier.c — Public Verifier for the 2-level Schnorr-HIBS scheme.
 *
 * Implements HIBS.Verify for k = 2. Using only public information
 * (mpk, the two delegation points, the identity strings, the message,
 * and the signature pair (h, s)), the verifier reconstructs the
 * effective public key implicitly committed to by the hierarchical
 * delegation chain, recovers the Schnorr commitment R', and checks
 * the Fiat-Shamir consistency h == H2(message || R').
 *
 * Verification math:
 *   c_ID1  = H1(ID_1 || Q_ID1)
 *   c_ID2  = H1(ID_2 || Q_ID1 || Q_ID2)
 *   PK_eff = (c_ID1 * c_ID2) * mpk  +  c_ID2 * Q_ID1  +  Q_ID2
 *   R'     = s * P  -  h * PK_eff
 *   Accept iff  h == H2(message || R')
 *
 * Input:
 *   argv[1] — ID1.txt        argv[5] — Q_ID1.txt
 *   argv[2] — ID2.txt        argv[6] — Q_ID2.txt
 *   argv[3] — message.txt    argv[7] — sig_s.txt
 *   argv[4] — mpk.txt        argv[8] — sig_h.txt
 *
 * Output:
 *   verification.txt — hex of the recomputed challenge h_check (debug aid)
 *   Console line:    "[verifier] Signature is VALID." | "... INVALID."
 */
...
int main(int argc, char **argv)
{
    ...
    // Load all public inputs: master public key, both delegation points, and (s, h)
    ...
    // Read identity strings ID_1, ID_2 and the message; trim trailing newline/CR
    ...
    // Allocate BIGNUM context used by modular arithmetic and EC operations
    ctx = BN_CTX_new();
    if(ctx == NULL) {
    ...
    // Recompute the level-1 identity-binding scalar c_ID1 = H1(ID_1 || Q_ID1) mod q
    size_t qid1_len;
    ...
    // Recompute the level-2 identity-binding scalar c_ID2 = H1(ID_2 || Q_ID1 || Q_ID2) mod q
    size_t qid2_len;
    ...
    //   PK_eff = (c_ID1 * c_ID2) * mpk  +  c_ID2 * Q_ID1  +  Q_ID2
    // This is the EC point whose discrete log under P equals sk_ID2.
    c1c2 = BN_new();
    if(c1c2 == NULL) {
    ...
    // Recover the Schnorr commitment R' = s * P - h * PK_eff.
    // Computed as s*P + (-(h * PK_eff)) using EC_POINT_invert for negation.
    ...
    // Recompute the challenge h_check = H2(message || R') mod q and persist for debug
    size_t Rprime_len;
    ...
    // Constant-time-equivalent equality check on the two challenge scalars
