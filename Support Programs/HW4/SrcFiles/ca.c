/*
Implementation of a file-based Kerberos authentication protocol demo in C, across four programs: client.c, kdc.c, tgs.c, 
and service.c. Provides mutual authentication and session key distribution using PKINIT (Public Key Cryptography for 
Initial Authentication in Kerberos), ephemeral Elliptic Curve Diffie-Hellman (ECDH) key exchange, and ECDSA digital 
signatures for client authentication. Session keys are derived via SHA-256 hashing of ECDH shared secrets, and all 
ticket and authenticator encryption uses AES-256-ECB symmetric encryption. Utilizes ECDSA, ECDH, SHA-256, and AES-256-ECB 
...
and service.c. Provides mutual authentication and session key distribution using PKINIT (Public Key Cryptography for 
Initial Authentication in Kerberos), ephemeral Elliptic Curve Diffie-Hellman (ECDH) key exchange, and ECDSA digital 
signatures for client authentication. Session keys are derived via SHA-256 hashing of ECDH shared secrets, and all ticket 
and authenticator encryption uses AES-256-ECB symmetric encryption. Utilizes ECDSA, ECDH, SHA-256, and AES-256-ECB functions 
