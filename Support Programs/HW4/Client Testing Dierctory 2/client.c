...
int main(int argc, char *argv[]) {
	 ...
	 */
	if(file_exists("Correct_AS_REP_2.txt") == 0){	// Cor
	...
	if(read_hex_file_bytes("Correct_Key_Client_AS_2.txt", &reference_key, &reference_key_len) == 0){	// Cor
	...
	if(aes256_decrypt_hex_file_to_bytes(key_client_as, "Correct_AS_REP_2.txt", &as_rep_plaintext, &as_rep_plaintext_len) == 0){		// Cor
	 ...
	 */
	if(file_exists("Correct_TGS_REP_2.txt") == 0){		// Cor
	 ...
	 */
	unsigned char* enc_key_client_app = read_line("Correct_TGS_REP_2.txt", 2);		// Cor
	...
	unsigned char* ticket_app_hex = read_line("Correct_TGS_REP_2.txt", 1);	// Cor
