 ...
 */
 ...
 /*
 * SERVICE.C - Application Server
 *
 * Invocation: ./service <APP_REQ.txt> <Key_TGS_App.txt>
 *
 * STEP 1 - ARGUMENT CHECK
 *   Verify exactly 2 command-line arguments are provided.
 *   If not, print usage message and exit with EXIT_FAILURE.
 *
 * STEP 2 - CHECK FOR SERVICE REQUEST
 *   Check if APP_REQ.txt exists.
 *   If missing, print "Service not requested yet" and exit.
 *   This ensures the server waits until a request is available.
 *
 * STEP 3 - READ THE TGS-APPLICATION SHARED KEY
 *   Read Key_TGS_App from the provided key file.
 *   The key must be exactly 32 bytes (256 bits).
 *   If the key is unreadable or the wrong length, terminate.
 *
 * STEP 4 - DECRYPT THE APPLICATION TICKET (AES-256-ECB)
 *   Read Ticket_App (hex) from line 1 of APP_REQ.txt.
 *   Decrypt it using AES-256-ECB with Key_TGS_App.
 *   Obtain plaintext: clientID_1 || Key_Client_App_hex
 *   If decryption fails or plaintext is too short, exit.
 *
 * STEP 5 - PARSE CLIENT ID AND CLIENT-APPLICATION KEY
 *   Split the ticket plaintext into:
 *     - clientID_1 (variable-length string prefix)
 *     - Key_Client_App_hex (trailing 64-character hex string = 32 bytes)
 *   Convert Key_Client_App_hex to binary Key_Client_App.
 *   If the hex format is invalid, terminate.
 *
 * STEP 6 - DECRYPT THE CLIENT AUTHENTICATOR (AES-256-ECB)
 *   Read Auth_Client_App (hex) from line 2 of APP_REQ.txt.
 *   Decrypt it using AES-256-ECB with Key_Client_App.
 *   Obtain clientID_2 from the decrypted authenticator.
 *   If decryption fails, exit.
 *
 * STEP 7 - VALIDATE THE CLIENT
 *   Compare clientID_1 (from ticket) with clientID_2 (from authenticator).
 *   If they do not match, print an error and exit.
 *   A match confirms the client possesses the correct session key.
 *
 * STEP 8 - RESPOND TO CLIENT
 *   Write APP_REP.txt with a single line: "OK"
 *   This confirms the client is authenticated and the request is accepted.
 *   If file writing fails, terminate.
 *   Successful write concludes the server-side protocol.
 */
