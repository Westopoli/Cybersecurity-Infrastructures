...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        for( ; iterations < pow(2, difficulty); iterations++) {
            nonce++;
        ...
        if(iterations >= pow(2, difficulty))
            break;
    }
    ...
    Print_Hex(label, (const unsigned char*)hex_nonce, strlen(hex_nonce));

    Write_File("solution_nonce.txt", hex_nonce, strlen(hex_nonce));
